...
def uploaded_files():
    ...
    if request.method == "POST":
        ...
        if uploaded_file.filename.lower().endswith((".txt")):
            ...
            uploaded_file_name = (
                str(uuid.uuid4()) + "__" + secure_filename(uploaded_file.filename)
            )
...
def get_one_random_line():
    ...
    file = open(latest_file, "r", encoding="utf-8")
    all_lines = {}
    ...
    try:
        ...
        return jsonify(
            {"message": "Please check previously uploaded file (It may be empty)"}
        )
...
def get_twenty_long_lines():
    ...
    """
    latest_file = get_latest_file(upload_path)
    with open(latest_file, encoding="utf-8") as l_file:
